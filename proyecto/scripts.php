...
<!--TABLES -->
<!--DataTables-->
<script src="assets/js/scripts-init/jquery.dataTables.min.js"></script>
...
$(document).ready(function() 			  
            {	
				...
				const pagina = URLactual.split("/");
				var ele=pagina.length;
				var pos=ele-1;
				console.log(ele);
				if (pagina[pos]=='login.php'||pagina[pos]=='login.php?error=1'||pagina[pos]=='login.php?error=2'||pagina[pos]=='carga_archivos_walmart.php'||pagina[pos]=='carga_archivos_cencosud.php'||pagina[pos]=='carga_archivos_smu.php')
					{
				  	console.log('si es')
					...
					$.post("notificaciones_ajax.php", {proceso:'LABOR'}, 
					   function(data) 
						...
						if (data=='OWNER')
							...
							alertaGeneral(nombre,"le recordamos que está trabajando con el cliente "+nombre,"warning");	
...
/*function notificaciones ()
	{
		...
		$.post("notificaciones_ajax.php", {proceso:'MENSAJES'}, 
			   function(data) 
			   	{
				$('body').append(data);
...
/*function listado_notificaciones ()
	{
		
		
		$.post("notificaciones_ajax.php", {proceso:'LISTADO_MENSAJES'}, 
			   function(data) 
			   	{
			console.log("lista de notificaciones ")
				$('#listado_notifi').html(data);
...
function notifica_campana ()
	{
		
		
		$.post("notificaciones_ajax.php", {proceso:'CAMPANA'}, 
			   function(data) 
			   	{
				console.log("camapa notifi")
				if (data!='')
					{
						console.log("si")
					document.getElementById('punto_campana').style.display='block';	
					}
				else
					{
						console.log("no")
					document.getElementById('punto_campana').style.display='none';	
						
					}
				}); 
		
	}
function notifica_ok(id)
	{
		$.post("notificaciones_ajax.php", {proceso:'NOTIFICA_OK',id:id}, 
			   function(data) 
			   	{
				}); 
...
function listado_clientes ()
	{
		...
		$.post("notificaciones_ajax.php", {proceso:'LISTADO_CLIENTES'}, 
			   function(data) 
			   	{
...
<div class="modal fade bd-example-modal-lg " id="modal_cambio_clave" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Cambio de clave</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body" id="texto_modal">
                <form action="" id="formResponderCaso" class="col-md-10 mx-auto" method="post" action="">
					<div class="position-relative form-group">
						<label for="clave_actual" class="col-sm-12 col-form-label">Ingrese clave actual:</label>
							<div class="col-sm-12"> 
								<input id="clave_actual" type= "password" class="form-control" >						
							</div>
					</div>
					<div class="position-relative form-group">
						<label for="nueva_clave1" class="col-sm-12 col-form-label">Ingrese nueva clave de 6 digitos:</label>
							<div class="col-sm-12"> 
								<input id="nueva_clave1" type= "password" class="form-control" val=''>					
							</div>
					</div>																			
        			<div class="position-relative form-group">
						<label for="nueva_clave2" class="col-sm-12 col-form-label">Ingrese nuevamente  clave de 6 digitos:</label>
							<div class="col-sm-12"> 
								<input id="nueva_clave2" type= "password" class="form-control" val=''>					
							</div>
					</div>																			
        	    </form>
            </div>
            <div class="modal-footer">
				<button type="button" style="margin-right: auto" class="btn btn-primary form-control-sm" name="btn_guardar" id="btn_guardar" onClick="cambiar_clave()"> Cambiar </button>
                <button type="button" class="btn btn-secondary form-control-sm" data-dismiss="modal">Cerrar</button>
            </div>
        </div>
    </div>
</div>
